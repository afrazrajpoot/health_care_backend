...
**CRITICAL: Report Origin Detection**
Before generating tasks, you MUST first determine if this report is:
- **EXTERNAL REPORT**: Generated by another clinic/physician that our clinic is reviewing
- **INTERNAL REPORT**: Generated by our own clinic for our own workflow

This distinction is CRITICAL as it completely changes what tasks should be generated.

---

## 🏥 REPORT ORIGIN DETECTION LOGIC

### **Step 1: Analyze the MATCHED_DOCTOR_NAME context**

If `MATCHED_DOCTOR_NAME` is provided and not empty:
- This doctor is from OUR clinic
- Check if this doctor is the PRIMARY treating physician OR a reviewing physician

### **Step 2: Examine the FULL_DOCUMENT_TEXT for origin indicators**

**EXTERNAL REPORT Indicators:**
- Document header shows different clinic/facility name
- "Consultation from [External Provider]"
- "Outside records review"
- "Second opinion from [External Specialist]"
- "Referred to/from [External Provider]"
- Treatment plans/procedures already performed at another facility
- Authorization/scheduling managed by external clinic
- Our doctor mentioned as "reviewing physician" or "consulting physician"
- Phrases like "Patient seen at [External Clinic]", "Per outside provider", "External report shows"

**INTERNAL REPORT Indicators:**
- Our clinic name in header/footer
- MATCHED_DOCTOR_NAME is the treating/authoring physician
- Document describes OUR clinic's examination, treatment plan, or procedures
- Internal workflow language (e.g., "Schedule with our office", "Our treatment plan")
- Authorization requests from OUR clinic
- No references to external providers as primary treaters

### **Step 3: Determine Clinical Context**

For EXTERNAL reports, detect:
- ✅ Has treatment already occurred? (past tense procedures, completed therapies)
- ✅ Is treatment actively managed elsewhere? (ongoing care at external facility)
- ✅ Are appointments already scheduled externally?
- ✅ Is authorization/approval already handled by the external provider?
...
**Your job:** Extract EACH distinct action that must happen based on the document's content AND the report origin.
...
## 🔢 TASK GENERATION RULES BY REPORT ORIGIN

### **🔴 EXTERNAL REPORT TASK GENERATION (MINIMAL & COORDINATION ONLY)**

When you determine the report is EXTERNAL, apply these strict rules:

**❌ DO NOT CREATE tasks for:**
- Scheduling treatments/procedures already managed externally
- Diagnostic studies ordered by external provider
- Therapeutic interventions performed or planned by external clinic
- Authorization requests handled by external provider
- Internal workflow tasks (intake, chart prep, rooming, clinical documentation)
...
1. **Care Coordination** (when explicitly needed):
   - "Coordinate with [External Clinic] regarding transfer of care"
...
**Example External Report Scenarios:**

**Scenario 1: External Orthopedic Consult (Already Completed)**
```
Report shows: "Patient seen by Dr. Smith at ABC Orthopedics on 12/01/2025. MRI ordered and scheduled for 12/15/2025 at ABC Imaging. Patient to follow up with Dr. Smith in 4 weeks."

✅ CORRECT Task Generation:
- Task 1: "Review external orthopedic recommendations" → Medical/Clinical
(Possibly no tasks if no action needed from our clinic)

❌ INCORRECT Task Generation:
- "Schedule MRI for patient" (Already scheduled externally)
- "Schedule orthopedic follow-up" (Already managed by external provider)
```

**Scenario 2: External PT Report (Treatment Ongoing)**
```
Report shows: "Patient completing 12-week physical therapy program at XYZ Therapy Center. Progress note shows 6/12 sessions completed. Next session scheduled 12/10/2025."

✅ CORRECT Task Generation:
- No tasks needed (External provider managing care)

❌ INCORRECT Task Generation:
- "Schedule physical therapy" (Already scheduled)
- "Monitor PT progress" (External provider responsibility)
```

**Scenario 3: External Specialist Recommendations Requiring Our Action**
```
Report shows: "Cardiologist Dr. Johnson recommends patient start beta-blocker therapy. Prescription to be managed by primary care physician."

✅ CORRECT Task Generation:
- Task 1: "Review cardiologist recommendations and prescribe beta-blocker" → Medical/Clinical

❌ INCORRECT Task Generation:
- "Schedule cardiology follow-up" (If not explicitly requested for our clinic)
```

---

### **🟢 INTERNAL REPORT TASK GENERATION (FULL WORKFLOW)**

When you determine the report is INTERNAL (from our clinic), use the FULL task generation logic:

**Generate ALL necessary tasks including:**
- Scheduling treatments/procedures/appointments
- Diagnostic study coordination
- Therapeutic interventions
- Authorization requests and tracking
- Internal workflow tasks (clinical, administrative, scheduling)
- Treatment planning and modifications
- Medication management
- Follow-up appointments
- Clinical reviews and assessments

Apply all the standard task generation rules, department routing, and urgency detection as described in the rest of this prompt.
...
Ask yourself:
1. ✅ Is this an EXTERNAL or INTERNAL report?
2. ✅ If EXTERNAL: Are the treatments/appointments already managed elsewhere?
3. ✅ If EXTERNAL: Am I generating ONLY minimal coordination tasks?
4. ✅ Have I identified ALL distinct actions in this document?
5. ✅ Is each task SPECIFIC with actual document details?
6. ✅ Is the department routing CORRECT for workflow ownership?
7. ✅ Is the due date APPROPRIATE for urgency level?
8. ✅ Does quick_notes.details explain WHY this matters?
9. ✅ Would a staff member know EXACTLY what to do?
10. ✅ Am I using ONLY information from the document?
11. ✅ Are multiple tasks truly distinct or should they be combined?
12. ✅ For EXTERNAL reports: Am I avoiding duplicate work already done externally?

---

**Remember:** 
- For EXTERNAL reports → Generate MINIMAL tasks (coordination/communication only)
- For INTERNAL reports → Generate FULL tasks (complete workflow)
- Quality and clinical appropriateness matter more than quantity.
"""
...
**CRITICAL FIRST STEP: Determine Report Origin**

Before generating ANY tasks, you MUST analyze:

1. **Is MATCHED_DOCTOR present?** 
   - If yes: Is this doctor the PRIMARY treating physician or a REVIEWING physician?
   - Check the FULL DOCUMENT TEXT for context clues

2. **Examine the FULL DOCUMENT TEXT for origin indicators:**
   - External clinic name in header/footer?
   - References to "outside provider", "external facility", "referred from"?
   - Treatment already performed elsewhere?
   - Appointments already scheduled at another clinic?
   - Our doctor mentioned as "reviewing" vs "treating"?

3. **Determine Clinical Context:**
   - Has treatment already occurred externally?
   - Is care actively managed by another provider?
   - Are there pending actions at the external facility?

**CLASSIFICATION:**
- If report is from EXTERNAL clinic → Apply MINIMAL task generation (coordination only)
- If report is from INTERNAL clinic (ours) → Apply FULL task generation (complete workflow)
...
2. What are ALL the distinct actions THIS CLINIC must take?
3. What actions are already handled externally that we should NOT duplicate?
4. Should these be separate tasks or combined into one?
5. Which department owns each action in our workflow?
6. What is the appropriate timeline based on urgency for each?
7. What specific details make each task actionable?
...
"""
    ...
    async def generate_tasks(self, document_analysis: dict, source_document: str = "", full_text: str = "", matched_doctor_name: str = "") -> list[dict]:

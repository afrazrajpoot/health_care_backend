...
class DocumentAnalysis(BaseModel):
    ...
    next_step: str = Field(default="Not specified", description="Recommended next steps in 2-3 words")
    adls_affected: str = Field(default="Not specified", description="Activities affected in 2-3 words")
    work_restrictions: str = Field(default="Not specified", description="Work restrictions in 2-3 words")
    document_type: str = Field(..., description="Type of document")
    summary_points: List[str] = Field(..., description="3-5 key points, each 2-3 words")
    
    # Optional fields to match your actual data structure
...
class ReportAnalyzer:
    ...
    def compare_with_previous_documents(
        ...
        Returns meaningful 'What's New' data from current analysis when no previous documents exist.
        """
        ...
        print("current data ", current_analysis)
          
        # ✅ FLEXIBLE TYPE HANDLING: Handle foreign Pydantic models or dicts with safe field mapping
        try:
            if hasattr(current_analysis, 'dict'):
                ...
                current_analysis = self._safe_convert_to_document_analysis(current_analysis_dict)
                logger.info(f"✅ Converted foreign Pydantic model to local DocumentAnalysis")
            elif isinstance(current_analysis, dict):
                current_analysis = self._safe_convert_to_document_analysis(current_analysis)
        ...
        # ✅ KEY FIX: If no previous documents, generate proper "What's New" from current analysis
        if not previous_documents:
            logger.info("✅ No previous documents: Generating 'What's New' from current analysis")
            return self._generate_whats_new_from_current(current_analysis, mm_dd)
        ...
        try:
            ...
            if isinstance(raw_result, str):
                try:
                    ...
                    # Fallback to generating from current analysis
                    return self._generate_whats_new_from_current(current_analysis, mm_dd)
            else:
            ...
            if not isinstance(result, dict):
                ...
                return self._generate_whats_new_from_current(current_analysis, mm_dd)  # Fallback to current analysis
            
            # Flatten any nested dicts/strings (handles cases like {"diagnosis": {"value": "str"}} -> {"diagnosis": "str"})
            flattened_result = {}
            ...
            if not merged_result:
                merged_result = self._generate_whats_new_from_current(current_analysis, mm_dd)
            logger.info(f"✅ MERGED 'What's New' (preserving history): {merged_result}")
            return merged_result
            
        except Exception as e:
            logger.error(f"❌ AI comparison failed: {str(e)}")
            # Fallback: Generate from current analysis
            return self._generate_whats_new_from_current(current_analysis, mm_dd)

    def _safe_convert_to_document_analysis(self, data_dict: Dict[str, Any]) -> DocumentAnalysis:
        """
        Safely convert a dictionary to DocumentAnalysis by mapping fields and providing defaults.
        """
        # Map your actual data fields to the expected DocumentAnalysis fields
        mapped_data = {
            'patient_name': data_dict.get('patient_name', 'Not specified'),
            'claim_number': data_dict.get('claim_number', 'Not specified'),
            'dob': data_dict.get('dob', datetime.now().strftime("%Y-%m-%d")),
            'doi': data_dict.get('doi', datetime.now().strftime("%Y-%m-%d")),
            'status': data_dict.get('status', 'normal'),
            'rd': data_dict.get('rd', datetime.now().strftime("%Y-%m-%d")),
            'diagnosis': data_dict.get('diagnosis', 'Not specified'),
    ...
    def _generate_whats_new_from_current(self, current_analysis: DocumentAnalysis, mm_dd: str) -> Dict[str, str]:
        """
        Generate meaningful 'What's New' data from the current analysis only.
        Creates proper categories based on document content.
        """
        whats_new = {}
        
        # Extract key information from current analysis - use safe access with defaults
        doc_type = getattr(current_analysis, 'document_type', '') or ""
        diagnosis = getattr(current_analysis, 'diagnosis', '') or ""
        key_concern = getattr(current_analysis, 'key_concern', '') or ""
        work_restrictions = getattr(current_analysis, 'work_restrictions', '') or ""
        next_step = getattr(current_analysis, 'next_step', '') or ""
        status = getattr(current_analysis, 'status', '') or ""
        claim_number = getattr(current_analysis, 'claim_number', '') or ""
        
        # Use extracted_recommendation if next_step is not available
        if next_step in ['Not specified', '']:
            extracted_rec = getattr(current_analysis, 'extracted_recommendation', '')
            if extracted_rec and extracted_rec not in ['Not specified', '']:
                next_step = extracted_rec
        
        # Determine document category and create appropriate "What's New" entries
        doc_type_lower = doc_type.lower()
        
        # diagnosis findings (for imaging reports, medical reports, progress reports)
        if any(word in doc_type_lower for word in ['mri', 'imaging', 'scan', 'ct', 'x-ray', 'progress', 'medical', 'report']):
            if diagnosis.lower() not in ['not specified', 'none', '']:
                # Use the actual diagnosis from your data
                whats_new['diagnosis'] = f"{diagnosis} ({mm_dd})"
            elif key_concern.lower() not in ['not specified', 'none', '']:
                whats_new['diagnosis'] = f"{key_concern} ({mm_dd})"
        
        # QME/Independent medical exams
        elif any(word in doc_type_lower for word in ['qme', 'evaluator', 'independent medical']):
            if diagnosis.lower() not in ['not specified', 'none', '']:
                whats_new['qme'] = f"{diagnosis} - {key_concern} ({mm_dd})"
            else:
                whats_new['qme'] = f"Medical evaluation - {key_concern} ({mm_dd})"
        
        # UR Decisions/Work restrictions
        elif any(word in doc_type_lower for word in ['ur', 'utilization review', 'work restriction', 'decision']):
            if work_restrictions.lower() not in ['not specified', 'none', '']:
                whats_new['ur_decision'] = f"{work_restrictions} ({mm_dd})"
            elif key_concern.lower() not in ['not specified', 'none', '']:
                whats_new['ur_decision'] = f"{key_concern} - {next_step} ({mm_dd})"
        
        # Legal documents
        elif any(word in doc_type_lower for word in ['legal', 'attorney', 'claim']):
            if claim_number and claim_number.lower() != 'not specified':
                whats_new['legal'] = f"Claim {claim_number} - {status} ({mm_dd})"
            else:
                whats_new['legal'] = f"Legal document - {key_concern} ({mm_dd})"
        
        # Default fallback - ensure we always have SOME meaningful data
        if not whats_new:
            if diagnosis.lower() not in ['not specified', 'none', '']:
                whats_new['diagnosis'] = f"{diagnosis} ({mm_dd})"
            elif key_concern.lower() not in ['not specified', 'none', '']:
                whats_new['diagnosis'] = f"{key_concern} - {next_step} ({mm_dd})"
            elif work_restrictions.lower() not in ['not specified', 'none', '']:
                whats_new['ur_decision'] = f"{work_restrictions} ({mm_dd})"
            else:
                # Absolute fallback - use document type
                whats_new['initial'] = f"Initial {doc_type} ({mm_dd})"
        
        logger.info(f"✅ Generated 'What's New' from current analysis: {whats_new}")
        return whats_new
    ...
    def create_whats_new_prompt(self) -> PromptTemplate:
        ...
        * diagnosis: Diagnosis changes, medical findings
        ...
        * legal: Legal developments, attorney letters, claim updates, whether approved or denied along with reason.
        ...
        - For diagnosis category, use the full diagnosis string from each analysis.diagnosis (which includes comma-separated key findings), chaining them in sorted date order.
        ...
        - First MRI report (analysis has "Date: 10/02"): {{"diagnosis": "Normal MRI, no mass lesion, clear sinuses (10/02)"}}
        ...
        - Diagnosis progression: {{"diagnosis": "lumbar strain (09/01) → lumbar strain, disc bulge, no compression (10/02)"}}
        ...
        - {{"diagnosis": "Lumbar Disc Herniation, Left L5 Radiculopathy, L4-L5 disc protrusion (05/15) → Lumbar Disc Herniation, Resolving L5 Radiculopathy (09/12)"}}
    ...
    def format_whats_new_as_highlights(self, whats_new_dict: Dict[str, str], current_date: str) -> List[str]:
        ...
        Maps internal keys to sample categories (e.g., 'diagnosis' → 'New diagnosiss').
        """
        mm_dd = datetime.strptime(current_date, "%Y-%m-%d").strftime("%m/%d")
        category_mapping = {
            "diagnosis": "New diagnosiss",
            "qme": "New consults",

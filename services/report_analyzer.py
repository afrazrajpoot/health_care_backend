...
class ReportAnalyzer:
    """
    Optimized orchestrator with LLM chaining.
    Simplified to use only ClinicalNoteExtractor for all document types.
    """
    ...
    def __init__(self, mode):
        """Initialize LLM and the ClinicalNoteExtractor."""
        self.llm = AzureChatOpenAI(
            azure_endpoint=CONFIG.get("azure_openai_endpoint"),
            api_key=CONFIG.get("azure_openai_api_key"),
            deployment_name=CONFIG.get("azure_openai_deployment"),
        ...
        # Initialize only the ClinicalNoteExtractor
        self.clinical_note_extractor = ClinicalNoteExtractor(self.llm)
        
        logger.info("✅ ReportAnalyzer initialized (ClinicalNoteExtractor only)")
    ...
    def extract_document(self, text: str) -> Dict[str, str]:
        ...
        try:
            ...
            # Stage 2: Direct extraction using ClinicalNoteExtractor
            result_dict = self._route_to_extractor(
                text=text,
                doc_type_str=doc_type_str,
                is_standard_type=is_standard_type,
    ...
    def _route_to_extractor(
        ...
        """
        Simplified routing: Always use ClinicalNoteExtractor for all document types.
        """
        logger.info(f"🎯 Using ClinicalNoteExtractor for {doc_type_str}")
